...
# --- Application Factory Pattern for SQLAlchemy ---
...
from models import User, Business, SalesRecord,InventoryItem,Creditor,Debtor,HirableItem,CompanyTransaction,RentalRecord,Company,FutureOrder,Customer

@login_manager.user_loader
def load_user(user_id):
    """
    Required Flask-Login function to load a user from the database.
    """
...
def sync_businesses():
    ...
    try:
        ...
        # --- PULLING DATA FROM ONLINE TO OFFLINE (Business and Inventory) ---
        print("--- Pulling Business and Inventory data from online to offline ---")

        # Pull businesses
        # Fetch data into a Pandas DataFrame
        online_businesses_df = pd.read_sql_table('businesses', online_engine)
        
        # Save to offline SQLite, replacing existing records to ensure local is up-to-date
        # This will update existing businesses and add new ones
        with offline_engine.connect() as conn:
            online_businesses_df.to_sql('businesses', conn, if_exists='replace', index=False)
            conn.commit() # Commit the transaction

        # Pull inventory items
        online_inventory_df = pd.read_sql_table('inventory_items', online_engine)
        with offline_engine.connect() as conn:
            online_inventory_df.to_sql('inventory_items', conn, if_exists='replace', index=False)
            conn.commit() # Commit the transaction
        
        # You might need to do this for other critical tables like Users (if you sync them)
        # online_users_df = pd.read_sql_table('user', online_engine) # Note: 'user' is your table name
        # with offline_engine.connect() as conn:
        #    online_users_df.to_sql('user', conn, if_exists='replace', index=False)
        #    conn.commit()
        ...
        # Select only unsynced sales records from the offline database
        ...
        if not unsynced_sales_df.empty:
            ...
            # You need a way to update `synced_to_remote` in `sales_records` table
            # Example using SQLAlchemy ORM (more robust than direct SQL update for many rows):
        ...
        # Update the last sync timestamp in the app configuration or a dedicated table
        # For simplicity, we'll just flash a message.
        # You might want to save this in a Business attribute or a global setting.
...
# Make sure you have the Business model imported
from app import db, Business
...
def super_admin_dashboard():
    ...
    # Render the template and pass the businesses list to it
    return render_template('super_admin_dashboard.html', businesses=all_businesses)
...
def create_default_admin():
    with app.app_context():
        try:
            ...
            super_admin_username = 'superadmin' # Define username here
            ...
            if not existing_super_admin:
                ...
                super_admin_password = 'password' # Define password here
